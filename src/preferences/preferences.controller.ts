...
export class PreferencesController {
  ...
  @ApiResponse({ status: 201, description: 'User preferences created successfully.' }) // Document success response
  ...
  @ApiResponse({ status: 404, description: 'Preferences not found for the given user ID.' })
  ...
  @ApiResponse({ status: 404, description: 'Preferences not found for the given user ID.' })
  async updatePreferences(@Param('userId') userId: string, @Body() createPreferenceDto: CreatePreferenceDto) {
    return this.preferencesService.createOrUpdate(userId, createPreferenceDto);
  }

  @Delete(':userId')
  @ApiOperation({ summary: 'Delete user preferences' })
  @ApiResponse({ status: 200, description: 'User preferences deleted successfully.' })
  @ApiResponse({ status: 404, description: 'Preferences not found for the given user ID.' })
